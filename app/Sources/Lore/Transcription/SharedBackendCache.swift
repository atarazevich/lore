...
/// TranscriptionEngine draw from here so only one instance per model+vocabulary
/// combination exists in memory.
///
...
final class SharedBackendCache {
    ...
    /// Ensure a backend is prepared for the given model and vocabulary.
    ...
    func prepare(
        ...
        guard self.model != model || self.vocabulary != vocab || backend == nil else {
            diagLog("[BACKEND-CACHE] already cached \(model.rawValue)")
            return
        }

        diagLog("[BACKEND-CACHE] loading \(model.rawValue)...")
        let newBackend = model.makeBackend(customVocabulary: vocab)
