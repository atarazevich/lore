...
struct DictationItem: Identifiable, Codable, Equatable, Sendable {
    ...
    ///
    /// With "Mark each item in the text" off (#198) the item arrives bare: the
    /// copied words are a plain paragraph and a picture is the bare path or
    /// filename. Where it lands and whether it travels do not change — only
    /// whether the prompt says what it is.
    ///
    ...
    func pasteText(for target: PasteTarget) -> String? {
        let tagged = RichInputSettings.tagsEnabled
        switch kind {
        case .text:
            guard let text, !text.isEmpty else { return nil }
            return tagged ? "<copied>\n\(text)\n</copied>" : text
        case .url:
            guard let text, !text.isEmpty else { return nil }
            return tagged ? "<link>\(text)</link>" : text
        case .image:
            guard let name = fileName(for: target) else { return nil }
            return tagged ? "<screenshot>\(name)</screenshot>" : name
        case .fileURL:
            guard let name = fileName(for: target) else { return nil }
            return tagged ? "<file>\(name)</file>" : name
        }
    }
